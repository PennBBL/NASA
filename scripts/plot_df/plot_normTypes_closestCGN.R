### This script creates lobe plots for volume, gmd, reho and alff, norming using a pooled mean and SD, and with 
...
# Read in the data
nasa_data <- read.csv("/home/ebutler/erb_data/nasa/nasa_strucRest_KoshaTaki.csv")

# Remove BJ... messes with interpretability of phantom plots, and should not be used for scaling
nasa_data <- nasa_data[nasa_data$subject_1 != "BJ", ]
...
####### -------------------- Norm -------------------- #######
### ----- Pooled ----- ###
brain <- colnames(nasa_data)[15:59]
...
nonroicols <- c("subid", "winterover", "subject_1", "Time", "sub_id", "ses_id", "group", "scanner", "PatientSex", "PatientSize", "PatientWeight", "PatientBirthDate", "AcquisitionDate", "PatientAgeYears")
TF_vec <- c()
...
colnames(phantom_summary_gmd) <- c("Region (Phantoms)", "CGN 2014 Mean", "CGN 2014 SD", "CHR Mean", "CHR SD", "HOB Mean", "HOB SD", "CGN 2017 Mean", "CGN 2017 SD")
phantom_summary_vol <- round_df(phantom_summary[9:15,], digits=0)
colnames(phantom_summary_vol) <- c("Region (Phantoms)", "CGN 2014 Mean", "CGN 2014 SD", "CHR Mean", "CHR SD", "HOB Mean", "HOB SD", "CGN 2017 Mean", "CGN 2017 SD")
phantom_summary_reho <- round_df(phantom_summary[18:24,], digits=4)
colnames(phantom_summary_reho) <- c("Region (Phantoms)", "CGN 2014 Mean", "CGN 2014 SD", "CHR Mean", "CHR SD", "HOB Mean", "HOB SD", "CGN 2017 Mean", "CGN 2017 SD")
phantom_summary_alff <- round_df(phantom_summary[28:34,], digits=0)
colnames(phantom_summary_alff) <- c("Region (Phantoms)", "CGN 2014 Mean", "CGN 2014 SD", "CHR Mean", "CHR SD", "HOB Mean", "HOB SD", "CGN 2017 Mean", "CGN 2017 SD")
phantom_summary_hippovol <- round_df(phantom_summary[38:45,], digits=2)
colnames(phantom_summary_hippovol) <- c("Region (Phantoms)", "CGN 2014 Mean", "CGN 2014 SD", "CHR Mean", "CHR SD", "HOB Mean", "HOB SD", "CGN 2017 Mean", "CGN 2017 SD")
...
for (i in 1:nrow(phantomed_df)) {
	...
	for (j in 1:ncol(phantomed_df)) {
		...
		if (roi %in% roicols) {
			if (phantomed_df[i, "winterover"] == "wo_2015") {
				if (phantomed_df[i, "scanner"] == "CGN") {
					if (phantomed_df[i, "group"] == "Crew") {
						phantomed_df[i, roi] <- (phantomed_df[i, roi] - phantom_summary[phantom_summary$ROI == roi, "Mean_CGN2014"])/phantom_summary[phantom_summary$ROI == roi, "SD_CGN2014"] 
					} else { # Controls
						if (phantomed_df[i, "Time"] == "t0") { # Use Phantoms CGN 2014
							phantomed_df[i, roi] <- (phantomed_df[i, roi] - phantom_summary[phantom_summary$ROI == roi, "Mean_CGN2014"])/phantom_summary[phantom_summary$ROI == roi, "SD_CGN2014"]
						} else { # t18, Use Phantoms CGN 2017
							phantomed_df[i, roi] <- (phantomed_df[i, roi] - phantom_summary[phantom_summary$ROI == roi, "Mean_CGN2017"])/phantom_summary[phantom_summary$ROI == roi, "SD_CGN2017"]
						}
					}
...
####### -------------------- Plot -------------------- #######
types <- c("gmd", "vol", "reho", "alff", "hippovol")
...
for (norm in c("pooled", "phantomed")) {
	for (type in types) {
		...
		csv.name <- paste0("/home/ebutler/plots/nasa_antartica/", type, pdf.info)
		...
		# Pick limits for plots
		lower_order <- 0
		upper_order <- 0
		if (norm == "pooled") { dfs <- c("crew_df", "control_df", "phantom_df")
		} else { dfs <- c("crew_df", "control_df") }

		for (i in 1:length(ROIlist)) {
			for (j in 1:length(dfs)) {
				if (dfs[[j]] == "crew_df" | dfs[[j]] == "control_df") {
					for (k in c("t0", "t12", "t18")) {
						boo <- get(dfs[[j]])
						boo <- boo[boo$Time == k, ]
						n.rows <- nrow(boo)
						current_low <- roundTo(mean(boo[, ROIlist[i]]) - (sd(boo[, ROIlist[i]])/sqrt(n.rows)), .5, "floor")
						current_high <- roundTo(mean(boo[, ROIlist[i]]) + (sd(boo[, ROIlist[i]])/sqrt(n.rows)), .5, "ceiling")
						if (current_low < lower_order) { lower_order <- current_low }
						if (current_high > upper_order) { upper_order <- current_high }
					}
				} else if (dfs[[j]] == "phantom_df" & norm == "pooled") {
					for (k in c("t1", "t2", "t3", "t4")) {
						boo <- get(dfs[[j]])
						boo <- boo[boo$Time == k, ]
						n.rows <- nrow(boo)
						current_low <- roundTo(mean(boo[, ROIlist[i]]) - (sd(boo[, ROIlist[i]])/sqrt(n.rows)), .5, "floor")
		...
		crew_plot <- createGGPlotImage(summaryDF_crew, "Time", crew_title, lower_order=lower_order, upper_order=upper_order, increment=.5, rois=FALSE)
		...
		control_plot <- createGGPlotImage(summaryDF_control, "Time", control_title, lower_order=lower_order, upper_order=upper_order, increment=.5, rois=FALSE)

		# Phantoms
		if (norm == "pooled") {
			phantom_df$Time <- factor(phantom_df$Time)
			phantom_df$group <- factor(phantom_df$group)
			summaryDF_phantom <- createSummaryDF(phantom_df, c("Time"), list(), pattern1=pattern1, pattern2=pattern2, ROIlist=ROIlist) 
			phantom_title <- paste0("Phantoms ", title.info, " - ", plot.title)
			phantom_plot <- createGGPlotImage(summaryDF_phantom, "Time", phantom_title, lower_order=lower_order, upper_order=upper_order, increment=.5, rois=FALSE)
		}
